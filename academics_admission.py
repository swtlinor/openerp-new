...
class admission_settings(osv.osv):
    ...
    _columns = {
        ...
        'essay': fields.text('Essay Instruction', required=True, help='Application Essay Instruction',
                             track_visibility="onchange"),
        'essay2': fields.text('Personal Statement', required=True, help='Personal Statement Instruction',
                              track_visibility="onchange"),
        ...
        'g_upload': fields.text('Graduate Upload Message', required=True, track_visibility="onchange"),        
...
class aun_applicant(osv.osv):
    ...
    def _needaction_domain_get(self, cr, uid, context=None):
        if self.pool.get('ir.model.access').check_groups(cr, uid, "academics.group_admis_staff"):
            return [('state', '=', 'submitted')]
        return False
 
#     def fields_get(self, cr, uid, fields=None, context=None):
#         term_obj = self.pool.get('aun.registrar.term')
#         term_ids = term_obj.search(cr, SUPERUSER_ID, [('admission_start', '<=',
#                                           datetime.now().strftime('%Y-%m-%d %H:%M:%S')),
#                                           ('admission_end','>=',datetime.now().strftime('%Y-%m-%d %H:%M:%S'))])
#         if not term_ids:
...
#         return super(aun_applicant, self).fields_get(cr, SUPERUSER_ID, fields, context)
    ...
    def create(self, cr, uid, vals, context=None):
        # patch chijioke.kalu@aun.edu.ng
        # TODO: Determine purpose of this Test, Fix in next release cycle
        # Possible Issue: A new Applicant (Graduate/Undergraduate) after registration
        # can re-login with the registered email
        ...
        if 'partner_id' in vals and vals['partner_id']:
            ...
            term_ids = app_obj.search(cr, uid, [('partner_id', '=', vals['partner_id']),
                                                ('sem_id', '=', vals['sem_id'])])
            if term_ids:
    ...
    def get_statement(self, cr, uid, context=None):
        ...
        if not settings_id:
            raise osv.except_osv(_('Contact the Administrator!'), _('Settings has not been created!'))
        personal_statement = settings_obj.browse(cr, SUPERUSER_ID, settings_id[0]).essay2
    ...
    def get_upload2(self, cr, uid, context=None):
        settings_obj = self.pool.get('admission.settings')
        settings_id = settings_obj.search(cr, SUPERUSER_ID, [])
        if not settings_id:
            raise osv.except_osv(_('Contact the Administrator!'), _('Settings has not been created!'))
        upload_msg2 = settings_obj.browse(cr, SUPERUSER_ID, settings_id[0]).g_upload
        return upload_msg2

    def _get_essay_upload_flag(self, cr, uid, ids, name, args, context):
        # import pdb; pdb.set_trace()
        # Check if the record exists in aun.applicant.attachment
        aaa_obj = self.pool.get('aun.applicant.attachment')
        aaa_id = aaa_obj.search(cr, SUPERUSER_ID, [('applicant_id', '=', ids[0])])

        res = {}
        # It's really simple, if we found it, there was an upload otherwise 'a written essay'
        if not aaa_id:
            # This record has no binary in aun_applicant_attachment
            res[ids[0]] = False
            context['default_essay_upload_flag'] = False  # redundant?
        else:
            # It has, get it
            res[ids[0]] = True
            context['default_essay_upload_flag'] = True  # redundant?

        return res

    def _less_get_essay_upload_flag(self, cr, ids, context):
        # patch by chijioke.kalu@aun.edu.ng
        # TODO: Finish this code, sister to _get_essay_upload_flag
        res = {}
        # import pdb; pdb.set_trace()
        # result = {'essay_upload_flag': False}
        return res
        
    _columns = {
        'student_id': fields.char('Student ID', size=16, readonly=False, write=['academics.group_admis_director'],
                                  track_visibility="onchange"),
        ...
        'maiden_name': fields.char('Maiden Name', size=32, trace_visibility="onchange"),
        ...
        'marital': fields.selection([('S', 'Single'), ('M', 'Married'), ('D', 'Divorced'), ('W', 'Widower')],
                                    'Marital status', required=True, track_visibility="onchange"),
        ...
        'city_lga': fields.many2one('res.country.city', 'Local Government Area', track_visibility='onchange'),
        'state_of_origin': fields.many2one('res.country.state', 'State of Origin', track_visibility="onchange", required=True),
        ...
        'app_prev_enr_term': fields.char('If yes, please specify year(s) and term(s) you have studied', size=64,
                                         track_visibility="onchange"),
        'alumni': fields.boolean('AUN Alumni?', track_visibility="onchange"),
        'alumni_stud_id': fields.char('Please Enter Your Undergraduate ID', track_visibility="onchange"),
        'no_of_siblings_uni': fields.selection([('0', 'None'), ('1','1'),
                                                ('2', '2'), ('3', '3'), ('3+', 'Above 3')],
                                               'No of siblings in AUN', track_visibility="onchange"),
        'essay_instruction': fields.text('Essay Instruction'),
        'personal_statement': fields.text('Personal Statement'),
        'essay': fields.text('Essay'),
        'essay2': fields.text('Personal'),
        'web_app': fields.boolean('Web Application'),
        'test_result_ids': fields.one2many('applicant.test', 'app_id', 'Results', track_visibility="onchange"),
        # fields.binary essay_upload is no longer used, see button action in xml
        # TODO: Delete on final release - chijioke.kalu@aun.edu.ng
        # 'essay_upload': fields.binary('Essay'),
        'essay_upload_flag': fields.function(_get_essay_upload_flag, type='boolean', string='Is'),
        'image': fields.binary('Image'),
        'image_medium': fields.function(_get_image, fnct_inv=_set_image,
                                        string="Medium-sized image", type="binary", multi="_get_image",
                                        store={'aun.applicant': (lambda self, cr, uid, ids, c={}: ids, ['image'], 10),
                                               },
        ...
        'sem_id': fields.many2one('aun.registrar.term', 'Application Term',
                                  domain=[('open_for_admission', '=', True)], required=True,
                                  track_visibility="onchange"),
        'term_admitted': fields.many2one('aun.registrar.term', 'Admission Term',
                                         domain=[('open_for_admission', '=', True)], track_visibility="onchange"),
        'major_id': fields.many2one('aun.registrar.major', 'Major', track_visibility="onchange"),
        'guardians': fields.one2many('aun.guardian', 'app_id', 'Guardians', required=True),
        ...
        # fields.binary recommendation1, recommendation2 are no longer used, see button action in xml
        # TODO: Delete on final release - chijioke.kalu@aun.edu.ng
        # 'recommendation1': fields.binary('Recommendation 1'),
        # 'recommendation2': fields.binary('Recommendation 2'),
        'type': fields.many2one('aun.application.type', 'Application Type', required=True,
                                track_visibility="onchange"),
        ...
        'editable': fields.boolean('Edit',
                                   help="Do not uncheck edit before saving the application. "\
                                        "It will be unchecked automatically."),
        ...
        'upload_msg2': fields.text('uploads'),
        # fields.binary upload1, upload2, upload3 are no longer used, see button action in xml
        # TODO: Delete on final release - chijioke.kalu@aun.edu.ng
        ...
        'local_govt': fields.char('LGA', readonly=True),  # Historic Purpose
    }

    def onchange_city_lga(self, cr, uid, ids, city_lga, context=None):
        if city_lga:
            state_of_origin = self.pool.get('res.country.city').browse(cr, uid, city_lga, context).state_id.id
            return {'value': {'state_of_origin': state_of_origin}}
        return {}
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'prov_end_date' in vals and vals['prov_end_date']:
            if vals['prov_end_date'] < str(date.today()):
                raise osv.except_osv(_('Invalid Date!'),
                                     _('Enter a provisional end date greater than or equal to today\'s date!'))
        ...
        name = ''
        try:
            first_name = vals['fname'].capitalize()
            if not first_name:  # first_name == False:
                first_name = ''
            name = first_name
            vals.update({'fname': first_name})   
        except:
            name = applicant.fname.capitalize()
            vals.update({'fname': applicant.fname.capitalize()})
        
        try:
            middle_name = vals['mname']
            if not middle_name:  # middle_name == False:
                middle_name = ''
            else:
                middle_name = middle_name.capitalize()
            name += ' ' + middle_name
            vals.update({'mname': middle_name})
        except:
            if applicant.mname:  # applicant.mname != False:
                name += ' ' + applicant.mname.capitalize()
                vals.update({'mname': applicant.mname.capitalize()})
        try:
            last_name = vals['lname'].capitalize()
            if not last_name:  # last_name == False:
                last_name = ''            
            name += ' ' + last_name
            vals.update({'lname': last_name})
        except:
            name += ' ' + applicant.lname.capitalize()
            vals.update({'lname': applicant.lname.capitalize()})
    ...
    def on_change_type(self, cr, uid, ids, type_id, major_id, context=None):
        ...
        if type_id:
            ...
            if major_id:
                mc = self.pool.get('aun.registrar.major.course').search(cr, SUPERUSER_ID,
                                                                        [('major_id', '=', major_id),
    ...
    def check_required_fields(self, cr, uid, ids, context=None):
        ...
        if not application.high_school:
            applic_cat = []      # this added function seperates the rquired field notification for different levels
            cr.execute("SELECT app_category from aun_application_type WHERE id=%s", (application.type.id,)) 
            for applic_cat_row in cr.fetchall():
                applic_cat.append(applic_cat_row[0])
                applic_cat_hash = str(applic_cat).strip('[u]').replace("'","")    
                if applic_cat_hash == 'undergraduate':
                    empty_fields.append('High School Information')
                if applic_cat_hash == 'graduate':
                    empty_fields.append('College/University Information')

        # patch by chijioke.kalu@aun.edu.ng
        # TODO: for now commented out, find issue this check will create if commented out
        """
        if not (application.recommendation1 or application.recommendation2):
            empty_fields.append('Recommendation')
        """

        if not application.guardians:
            applic_cat = []  
            cr.execute("SELECT app_category from aun_application_type WHERE id=%s", (application.type.id,)) 
            for applic_cat_row in cr.fetchall():
                applic_cat.append(applic_cat_row[0])
                applic_cat_hash = str(applic_cat).strip('[u]').replace("'", "")
                if applic_cat_hash == 'undergraduate':
                    empty_fields.append('Parent/Guardian Information')
                if applic_cat_hash == 'graduate':
                    empty_fields.append('Next of Kin Information')
        ...
        if not application.test_result_ids:
            ...
            for applic_cat_row in cr.fetchall():
                ...
                if applic_cat_hash == 'graduate':
                    empty_fields.append('Degree Awarded')
        ...
        if not (application.essay or application.essay_upload):
            ...
            for applic_cat_row in cr.fetchall():
                ...
                if applic_cat_hash == 'graduate':
                    empty_fields.append('Personal Statement')
        """
        ...
        if uploads < no_of_uploads:
            raise osv.except_osv(_('Check uploads!'), _(str(
                no_of_uploads - uploads) +
                ' upload(s) missing under personal information (Step 1). Please upload the ' + str(
                no_of_uploads) + ' required document(s).'))
        """

        for test in application.test_result_ids:
            if not test.is_graduate: 
        ...
        for high_school in application.high_school:
            if not high_school.is_graduate:
        ...
        if empty_fields or no_subjects:
            raise osv.except_osv(_('Application Incomplete!'),
                                 _('The following information is missing: \n' + ', '.join(empty_fields) + '.' +
                                   ['\nSubjects for: \n',''][not no_subjects] + ', '.join(no_subjects)))
    

        if application.type.app_category != "foundation_program":
            if not application.signature:
                raise osv.except_osv(_('Signature Required!'),
                                     _('Please enter your full name as digital signature on Step 8 of the application.'))
    ...
    def case_submit(self, cr, uid, ids, context=None):
        ...
        if not applicant.date_submitted:
            self.write(cr, uid, ids, {'state': 'submitted', 'date_submitted': time.strftime('%Y-%m-%d %H:%M:%S')},
                       context=context)
        ...
        if self.pool.get('ir.model.access').check_groups(cr, uid, "academics.group_admis_applicant"):
            partner_obj.write(cr, SUPERUSER_ID, [applicant.partner_id.id], {
                                                     ...
                                                     'sex': applicant.sex,
                                                     'state_of_origin': applicant.state_of_origin.id,
    ...
    def case_submit_foundation(self, cr, uid, ids, context=None):
        self.check_required_fields_foundation(cr, uid, ids)
        applicant = self.browse(cr, uid, ids)[0]
        if not applicant.date_submitted:
            self.write(cr, uid, ids, {'state': 'submitted', 'date_submitted': time.strftime('%Y-%m-%d %H:%M:%S')},
                       context=context)
    ...
    def case_refuse(self, cr, uid, ids, context=None):
        self.write(cr, uid, ids, {'state': 'refused', 'date_refused': date.today(), 'provisional': False},
                   context=context)
    ...
    def case_accept(self, cr, uid, ids, context=None):
        ...
        if applicant.prev_student_id:
            applicant_partner_id = partner_obj.search(cr, SUPERUSER_ID, [('name', '=', applicant.prev_student_id),
                                                                         ('student', '=', True)])[0]
        else:
            if applicant.partner_id:
                applicant_partner_id = applicant.partner_id.id
                partner_obj.write(cr, SUPERUSER_ID, [applicant_partner_id], {'name': stud_id, 'fname': applicant.fname,
                                                                             'mname': applicant.mname,
                                                                             'lname': applicant.lname,
                                                                             'applicant': True})
            else:
                applicant_partner_id = partner_obj.create(cr, SUPERUSER_ID, {'name': stud_id, 'fname': applicant.fname,
                                                                             'mname': applicant.mname,
                                                                             'lname': applicant.lname,
                                                                             'applicant': True}, context=context)
        ...
        if self.browse(cr, uid, ids)[0].type.app_category == "graduate":
            self.email_applicant(cr, uid, ids, 'graduate_acceptance_email')            
        elif self.browse(cr, uid, ids)[0].type.app_category == "undergraduate":
        ...
        return self.write(cr, uid, ids, {'student_id': stud_id, 'partner_id':applicant_partner_id, 'state': 'accepted',
                                         'date_accepted': time.strftime('%Y-%m-%d %H:%M:%S')}, context=context)
    
    def create_stud_account(self, cr, uid, ids, context=None):
        applicants= self.browse(cr, SUPERUSER_ID, self.search(cr, SUPERUSER_ID, [('sem_id', '=', 118),
        ...
        for applicant in applicants:
            if applicant.prev_student_id:
                ...
                if applicant.partner_id:
                    applicant_partner_id = applicant.partner_id.id
                    partner_obj.write(cr, SUPERUSER_ID, [applicant_partner_id], {'name': applicant.student_id,
                                                                                 'fname': applicant.fname,
                                                                                 'mname': applicant.mname,
                                                                                 'lname': applicant.lname})
                else:
                    applicant_partner_id = partner_obj.create(cr, SUPERUSER_ID, {'name': applicant.student_id,
                                                                                 'fname': applicant.fname,
                                                                                 'mname': applicant.mname,
                                                                                 'lname': applicant.lname},
                                                              context=context)
    ...
    def email_applicant(self, cr, uid, ids, template_id, context=None):
        ...
        for record in self.browse(cr, uid, ids, context):
            ...
            if mail_state and mail_state['state'] == 'exception':
                raise osv.except_osv(_("Check email"),
                                     ("Cannot send email: Please check the applicant\'s email address. \n " +
    ...
    def case_admit(self, cr, uid, ids, context=None, *args):
        ...
        if not applicant.term_admitted:
            raise osv.except_osv(_('No Admission Term!'),
                                 _('You must pick the term for which the student is being admitted!'))
        ...
        catalogue_id = catalogue_obj.search(cr, SUPERUSER_ID,
                                            [('start_year', '<=', str(int(term_year) + int(term_term.year_adjustment))),
                                             ('end_year', '>', str(int(term_year) + int(term_term.year_adjustment)))])
        if not catalogue_id:
            raise osv.except_osv(_('Contact the Registrar!'),
                                 _('There is no matching catalogue for the applicant\'s admission year!'))
        ...
        if applicant.major_id and not applicant.major_id.no_school:
            ...
            major_ids = mc_obj.search(cr, SUPERUSER_ID, [('major_id', '=', applicant.major_id.id),
                                                         ('level_id', '=', level_id),
                                                         ('catalogue_id', '=', catalogue_id)])
            
            major_ids = mc_obj.browse(cr,SUPERUSER_ID,major_ids)
            if not major_ids:
                raise osv.except_osv(_('Major Unavailable!'),
                                     _(applicant.major_id.name +
                                       ' is not in the admission catalogue '
                                       'for this student! Please select another major!'))
                
        default_standing_id = False
        if major_ids[0].program_id.default_standing_id:
            default_standing_id = major_ids[0].program_id.default_standing_id.id
        elif applicant.type.level_id.default_standing_id:
            default_standing_id = applicant.type.level_id.default_standing_id.id
        ...
        res = {
                ...
                'state_of_origin': applicant.state_of_origin.id,
                'local_govt': applicant.local_govt,
                'sem_id': applicant.sem_id.id,
                'street': applicant.street,
                'street2': applicant.street2,
                'city': applicant.city,
                ...
                'standing_id': default_standing_id,
        ...
        stud_obj = self.browse(cr, uid, ids)[0]
     
        if applicant.prev_student_id:
            res['date_of_state'] = False
            res['graduated'] = False
        else:   
            res['name'] = applicant.student_id
            res['email'] = False
            res['student'] = True
            
        # copy guardian information from applicant guardian object to student guardian object
        rg_obj = self.pool.get('res.guardian')
        for guardian in applicant.guardians:
            rg_obj.create(cr, uid, {
                                    'name': guardian.name,
                                    ...
                                    'add': guardian.street + " " + [guardian.street2, ""][not guardian.street2],
                                    # [guardian.street2 == False],
                                    'city': guardian.city,
                                    'state': guardian.state_id.id,
                                    'country': guardian.country_id.id,
                                    'deceased': guardian.deceased,
                                    'partner_id': applicant_partner_id
                                })
        partner_obj.write(cr, SUPERUSER_ID, [applicant_partner_id], res)
        ...
        self.write(cr, uid, ids, {'state': 'done', 'date_admitted': time.strftime('%Y-%m-%d %H:%M:%S')},
                   context=context)
...
class aun_application_type(osv.osv):
    ...
    _columns = {
        ...
        'app_category': fields.selection([('undergraduate', 'Undergraduate'), ('graduate', 'Graduate'),('foundation_program','Foundation Program') ],
                                         'Category', required=True,
                                         help='Select the level, this is needed to sort application form',
                                         track_visibility="onchange"),
...
class aun_guardian(osv.osv):
    ...
    _columns = {
        'name': fields.selection([('aunt', 'Aunt'), ('brother', 'Brother'), ('child', 'Child'),
                                  ('father', 'Father'), ('friend', 'Friend'), ('grandparent', 'Grandparent'),
                                  ('guardian', 'Guardian'), ('husband', 'Husband'), ('mother', 'Mother'),
                                  ('other', 'Other'), ('sister', 'Sister'), ('uncle', 'Uncle'), ('wife', 'Wife'),
                                  ('other', 'Other')], 'Relationship Type', required=True),
        'prefix': fields.selection([('ms', 'Miss'), ('mst', 'Master.'), ('mr', 'Mr.'), ('mrs', 'Mrs.'), ('dr', 'Dr.'),
                                    ('chief', 'Chief'), ('alhaji', 'Alhaji'), ('hajiya', 'Hajiya'),
                                    ('sir', 'Sir')], 'Prefix'),
...
class aun_high_school_app(osv.osv):
    ...
    _columns = {
        ...
        'state_id': fields.related('name_search', 'state_id', type='many2one', relation='res.country.state',
                                   string="State", readonly=True, store=True),
        'country_id': fields.related('name_search', 'country_id', type='many2one', relation='res.country',
        ...
        'is_graduate': fields.boolean('Graduate?'),
        # fields.binary transcript_upload is no longer used, see button action in xml
        # TODO: Delete on final release - chijioke.kalu@aun.edu.ng
        # 'transcript_upload': fields.binary('Transcript Upload'),
        'transcript_ids': fields.one2many('transcript.course.app', 'high_school_app_id', 'Transcript Subjects',
                                          track_visibility="onchange"),
...
class test_name(osv.osv):
    ...
    _columns = {
        ...
        'active': fields.boolean('Active'),
        'admission_check': fields.boolean('Admission'),
        'registrar_check': fields.boolean('Registrar')
        }
    ...
    def _uniq_name(self, cr, uid, ids, context=None):
        ...
        for t in self.browse(cr, uid, test_ids):
            if t.name.lower() == test.name.lower():
                raise osv.except_osv(_('Check test name!'), _('There is another test called: ' +
                                                              t.name + '. The name must be unique.'))
        return True
...
class degree_name(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        vals['name'] = vals['name'].strip()
        vals['code'] = vals['code'].strip()
        return super(degree_name, self).create(cr, uid, vals, context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'code' in vals:
            vals['code'] = vals['code'].strip()
        return super(degree_name, self).write(cr, uid, ids, vals, context=context)

    def unlink(self, cr, uid, ids, context=None):
        super(degree_name, self).write(cr, uid, ids, {'active': False}, context=context)
        return True
   
    _columns = {
        'name': fields.char('Degree Name', size=64, required=True, track_visibility="onchange"),
        'code': fields.char('Degree Code', size=16, required=True, track_visibility="onchange"),
        'active': fields.boolean('Active')
        }
    ...
    def _uniq_degree(self, cr, uid, ids, context=None):
        degree = self.browse(cr, uid, ids, context=context)[0]
        degree_ids = self.search(cr, uid, [('id', 'not in', [degree.id])])
        for t in self.browse(cr, uid, degree_ids):
            if t.name.lower() == degree.name.lower():
                raise osv.except_osv(_('Check degree name!'), _('There is another degree called: ' +
                                                                t.name + '. The name must be unique.'))
        return True
    
    _constraints = [
        (_uniq_degree, 'There is another degree with the same name.', ['Name'])
    ]
...
class test_code(osv.osv):
    ...
    _columns = {
        ...
        'score_type': fields.selection([('N', 'Numeric'), ('A', 'Alphanumeric ')], 'Data Type', required=True,
                                       track_visibility="onchange"),
        ...
        'scores': fields.char('Scores Allowed(Optional)', size=256,
                              help='If there are specific scores allowed in the range, ' +
                                   'enter them here separated by comma e.g. A1, A2, B2, F9...',
                              track_visibility="onchange"),
        'active': fields.boolean('Active')
        }
    ...
    def _uniq_test_subject(self, cr, uid, ids, context=None):
        ...
        for t in self.browse(cr, uid, test_ids):
            if t.subject.lower() == test.subject.lower():
                raise osv.except_osv(_('Check test subject!'),
                                     _(t.subject + ' already exists for this test. Test subjects must be unique.'))
        return True
    ...
    def _check_min_max(self, cr, uid, ids, context=None):
        ...
        if test.score_type == 'N':
            ...
            if len(test.low) > test.positions: 
                raise osv.except_osv(_('Check min/max!'),
                                     _('The length of the min value must be less than or equal to the positions.'))
            if len(test.high) != test.positions:
                raise osv.except_osv(_('Check min/max!'),
                                     _('The length of the max value must be equal to the positions.'))
        ...
        if test.score_type == 'A':
            ...
            if len(test.low) != test.positions or len(test.high) != test.positions:
                raise osv.except_osv(_('Check min/max!'),
                                     _('The length of the min and max values must be equal to the positions.'))
            ...
            for i in xrange(len(test.low)):
                ...
                if ((test.low[i] in ascii_uppercase and test.high[i] in ascii_lowercase) or
                    ...
                    raise osv.except_osv(_('Check min/max!'),
                                         _('Ensure that both min and max have the same case (Uppercase or lowercase).'))
                
                # check if both characters are digits
                if (test.low[i].isdigit() and not test.high[i].isdigit()) or \
                        (test.high[i].isdigit() and not test.high[i].isdigit()):
                    raise osv.except_osv(_('Check min/max!'),
                                         _('Ensure that both min and max have digits in the same positions.'))
        ...
        if test.scores:
            ...
            for score in scores:
                self.validate_score(test.test_id.code + ' ' +
                                    test.subject, score, test.low, test.high, test.score_type)
    ...
    def validate_score(self, test_name, score, min_score, max_score, score_type):
        if score_type == 'N':
            try:
                ...
                raise osv.except_osv(_('Invalid Score!'),
                                     _(score + ' is not numeric and ' + test_name + ' is a numerically scored test.'))
            if len(score) > len(max_score):
                raise osv.except_osv(_('Check Score!'),
                                     _(score + ' is not valid for ' + test_name + '. Check number of digits.'))
            if not(float(max_score) >= float(score) >= float(min_score)):
                raise osv.except_osv(_('Invalid Score!'),
                                     _(score + ' is not in the range specified for ' + test_name + '.'))
        
        if score_type == 'A':
            if len(score) != len(min_score):
                raise osv.except_osv(_('Invalid Score!'),
                                     _(score + ' is not valid for ' + test_name + '. Check score length'))
            ...
            for i in xrange(len(min_score)):
                if min_score[i].isdigit():
                    if not score[i].isdigit():
                        raise osv.except_osv(_('Invalid Score!'),
                                             _(score + ' is not an acceptable score for ' + test_name))
...
class applicant_test(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for tr in app_test.test_result_ids:
            other_res_ids = self.pool.get('test.result').search(cr, uid, [('app_test_id', '=', tr.app_test_id.id),
                                                                          ('test_id', '=', tr.test_id.id)])
        ...
        if wrong_tests:
            raise osv.except_osv(_('Invalid Test!'),
                                 _('The following tests are not ' +
                                   app_test.test_name_id.code + ' tests: ' + ', '.join(wrong_tests)))
        if duplicate_tests:
            raise osv.except_osv(_('Duplicate Test!'),
                                 _('The following tests are duplicated: ' + ', '.join(set(duplicate_tests))))
        return res
    ...
    def _get_score_line(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get('test.result').browse(cr, uid, ids, context=context):
            result[line.app_test_id.id] = True
        return result.keys()
    
    def _get_sum(self, cr, uid, ids, field_name, arg=None, context=None):
        res = {}
        for app_test in self.browse(cr, uid, ids):
            total = 0
            for result in app_test.test_result_ids:
                if result.test_id.score_type == 'N':
                    total += float(result.score)
            # self.write(cr, uid, res, {'total_stored': total})
            res[app_test.id] = total
        return res
    
    def _get_score_type(self, cr, uid, ids, field_name, arg=None, context=None):
        res = {}
        is_numeric = True
        for app_test in self.browse(cr, uid, ids):
            for result in app_test.test_result_ids:
                if result.test_id.score_type == 'A':
                    is_numeric = False
            res[app_test.id] = is_numeric
        return res

    _columns = {
        'app_id': fields.many2one('aun.applicant', 'Student', required=True, track_visibility="onchange"),
        'test_name_id': fields.many2one('test.name', 'Examination',  track_visibility="onchange"),
        'student_id': fields.related('app_id', 'student_id', relation='aun.applicant', type='char', string='Student ID'), #Miriam
        # *********************
        # Developer Comments
        # *********************
        # fields.binary will not be modified in this class "applicant_test()"
        # see also supporting academics_admission_view.xml for this class
        #
        # Reason:
        # 'test' field column and supporting xml section <field name="test" required="1"/> stores for each
        # Placement Test, Exam Number an attached File (BLOB) which have multiple instances for an applicant
        # as indicated in the database table by the column app_id
        #
        # Database Table affected: applicant_test
        #
        # Resolution:
        # At the moment the BLOB size in the database do not pose a performance issue to the whole
        # application solution and therefore ignored for migration to the new file system data
        # storage methodology in this release cycle.
        # TODO: Fix this on next release cycle - chijioke.kalu@aun.edu.ng        
        'test': fields.binary('Upload'),
        'degree_name_id': fields.many2one('degree.name', 'Degree Awarded', track_visibility="onchange"),
        'degree_class': fields.char('Class of Degree', size=64),
        'courseof_study': fields.char('Course of Study', size=64),
        'cgpa': fields.float('CGPA (Undergraduate Only)', size=64),
        'exam_no': fields.char('Examination Number', size=64),
        ...
        'total_num': fields.function(_get_sum, type='float', method=True, string='Total Score',
                                     store={
                                         'applicant.test': (lambda self, cr, uid, ids, c={}: ids,
                                                            ['test_result_ids'], 10),
                                         'test.result ': (_get_score_line, ['score'], 10)}),
        'is_numeric': fields.function(_get_score_type, type='boolean', method=True, string='Numeric?', store=False),
        'is_graduate': fields.boolean('Graduate?'),       
        'active': fields.boolean('Active'),
        'jamb_reg_no': fields.char('Registration Number (JAMB Only)', size=64)
        }
...
#         return res
    ...
    _columns = {
        ...
        'app_id': fields.related('app_test_id', 'app_id', type='many2one', relation='aun.applicant',
                                 string='Applicant', readonly=True, store=True),
        'active': fields.boolean('Active')
        }
    ...
    def _check_score(self, cr, uid, ids, context=None):
        ...
        if test.scores:
            if result.score not in test.scores.replace(' ', '').split(','):
                raise osv.except_osv(_('Invalid Score!'),
                                     _(result.score + ' is not an acceptable score for ' + test.name + '.'))
        else:
...
class transcript_course_app(osv.osv):
    ...
    _columns = {
        ...
        'app_id': fields.related('high_school_app_id', 'app_id', type='many2one',
                                 relation='aun.applicant', string='Applicant', readonly=True, store=True),
        'active': fields.boolean('Active')
        }
    ...
    def _check_score(self, cr, uid, ids, context=None):
        ...
        subject = result.name
        if result.score < 0:
            raise osv.except_osv(_('Invalid Transcript Score!'),
                                 _('The score for ' + subject.name + ' cannot be less than 0.'))
        if result.score > subject.max:
            raise osv.except_osv(_('Invalid Transcript Score!'),
                                 _('The score for ' + subject.name + ' cannot be greater than '
                                   + str(subject.max) + '.'))
        return True
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if orig_ap_id:
            if self.browse(cr, uid, orig_ap_id)[0].new_password != vals['orig_password']:
                raise osv.except_osv(_('Incorrect Password'),
                                     _('The old password you provided is incorrect, your password was not changed.'))
